...
function mapModel(anthropicModel) {
  // 全Anthropicモデルを gemini-2.5-flash に統一マップ
  // （gemini-2.0-flash の Free tier quota は地域・プロジェクト依存で動かないケースがあるため）
...
async function createWithRetry(params, maxRetries = 3) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      const isRateLimit = status === 429 || /rate limit|quota|exceed/i.test(msg);
      // quota が完全に枯渇している（limit:0）場合は即座に投げる（無駄な待機回避）
      if (isRateLimit && /limit:\s*0/i.test(msg)) {
        console.error(`[gemini-client] quota=0で即終了: ${msg.slice(0, 200)}`);
        throw err;
      }
      if (isRateLimit && attempt < maxRetries) {
        // retryDelay ヘッダーまたはレスポンスに記載があればそれを使う、なければ短めに
        const m = msg.match(/retry in ([\d.]+)s/i);
        const suggestedSec = m ? Math.ceil(parseFloat(m[1])) : null;
        const waitSec = Math.min(suggestedSec || (15 * (attempt + 1)), 60);
      ...
      if (status && status >= 500 && attempt < 1) {
        console.log(`[gemini-client] ${status}エラー。15秒待機してリトライ...`);
