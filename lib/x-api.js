...
// リプライを投稿
async function replyToTweet(text, inReplyToTweetId) {
  try {
    const url = 'https://api.x.com/2/tweets';
    ...
    if (result.status === 201 || result.status === 200) {
      const tweet = result.data.data;
      console.log(`[replyToTweet] リプライ成功: ${tweet.id} → ${inReplyToTweetId}`);
      return { id: tweet.id, text: tweet.text };
    }

    handleApiError(result, 'replyToTweet');
    return null;
  } catch (err) {
    console.error(`[replyToTweet] エラー: ${err.message}`);
    return null;
  }
}
...
async function likeTweet(tweetId) {
  try {
    ...
    if (meResult.status !== 200 || !meResult.data?.data?.id) {
      console.error('[likeTweet] user_id取得失敗');
      return false;
    }
    const userId = meResult.data.data.id;

    const url = `https://api.x.com/2/users/${userId}/likes`;
    const result = await makeRequest('POST', url, { tweet_id: tweetId });

    if (result.status === 200 || result.status === 201) {
      console.log(`[likeTweet] いいね成功: ${tweetId}`);
      return true;
    }

    handleApiError(result, 'likeTweet');
    return false;
  } catch (err) {
    console.error(`[likeTweet] エラー: ${err.message}`);
    return false;
  }
}

module.exports = { postTweet, quoteTweet, getTweetMetrics, uploadMedia, postTweetWithMedia, replyToTweet, likeTweet };
